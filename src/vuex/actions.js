...
export const getCategory = ({ commit, dispatch }, payload) => {
  ...
  const route = `congress/sessions?private=false&$sort[startDateTime]=1`
  HTTP
  .get(route)
  .then(response => {
    data.items = response.data.data
    data.skip = response.data.skip
...
export const getPresentations = ({ commit, dispatch }, payload) => {
  return new Promise((resolve, reject) => {
    const route = `congress/presentations?sessionID=${payload}&$sort[startDateTime]=1`
    let data = {}
    ...
    .then(response => {
      ...
      data.items = response.data.data
      dispatch('setOnline')
      commit(types.SET_PRESENTATIONS, data, err => { console.log(err) })
      resolve(data)
    }).catch(e => {
      if (window.localStorage.getItem('vuex')) {
        const restored = JSON.parse(window.localStorage.getItem('vuex'))
